...
###############################################################################
#                                                                             #
#                      CONVERT_SMS_MMS_SQLITE_TO_XML.SH                       #
#                                                                             #
###############################################################################
...
#     Description:    Export mmssms.db (sqlite) from android phone to XML 
...
#----------------------------
    ...
    MODE_CHOICE="null"
    SCRIPT_NAME="CONVERT_SMS_MMS_SQLITE_TO_XML.sh"
    JOUR=`date +%Y_%m_%d`   
...
function main() {
    ...
    # Init var
    #---------------------------
    FILE_CSV="${FILE_SQLITE/'.db'/'.csv'}"
  #  echo $PATH_FILE_DIR
    PATH_FILE_CSV="$PATH_FILE_DIR/$FILE_CSV"
    FILE_XML_OUTPUT="${FILE_SQLITE/'.db'/}_$JOUR.xml"
    
    # Given file output    
    if [[ -n ${output_file-} ]]; then
        FILE_XML_OUTPUT=$output_file_param
    ...
    if [ $MODE -eq 1 ]; then
        [[ ! -z ${verbose-} ]] \
            && pretty_print \
            "$SCRIPT_NAME: Making XML file : mode SuperBackup"
        convert_callslog_to_xml "$PATH_FILE_CSV" \
        "$PATH_FILE_DIR/$FILE_XML_OUTPUT" \
        "SuperBackup"
    fi
    ...
    if [ $MODE -eq 2 ]; then
        [[ ! -z ${verbose-} ]] \
            && pretty_print \
            "$SCRIPT_NAME: Making XML file : mode BackupAndRestore"
        convert_callslog_to_xml "$PATH_FILE_CSV" \
        "$PATH_FILE_DIR/$FILE_XML_OUTPUT" \
        "BackupAndRestore"
    fi
